/*
  # Add event types and registration system
  
  1. Changes
    - Add registration status and state enums
    - Update event status enum to include 'ongoing'
    - Add event type enum and column
    - Add songs table for festival repertoire
    - Add registrations table with related fields
    - Add storage buckets for documents
...
UPDATE events e
SET status = CASE 
  WHEN t.old_status = 'upcoming' THEN 'upcoming'::event_status
...
ALTER TABLE events 
ADD COLUMN type event_type NOT NULL DEFAULT 'competition';
