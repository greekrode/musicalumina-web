/* # Database Schema Update 1. New Types - registration_status enum ('personal', 'parents', 'teacher') - registration_state enum ('pending', 'approved', 'rejected') - event_type enum ('festival', 'competition', 'masterclass') - event_status enum ('upcoming', 'ongoing', 'completed') 2. New Tables - songs: For storing song information - registrations: For event registrations 3. Storage - Added buckets for registration documents AND payment receipts - Added RLS policies for secure access */
...
ALTER TABLE events ADD COLUMN IF NOT EXISTS type event_type NOT NULL DEFAULT 'competition', ADD COLUMN IF NOT EXISTS status event_status NOT NULL DEFAULT 'upcoming';
-- Step 4:
...
-- Payment info
, bank_name text NOT NULL, bank_account_number text NOT NULL, bank_account_name text NOT NULL, payment_receipt_url text NOT NULL
-- Metadata
, status registration_state NOT NULL DEFAULT 'pending', created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now() );
-- Step 6:
...
DROP POLICY IF EXISTS "Authenticated users can read own registrations"
ON registrations;

DROP POLICY IF EXISTS "Authenticated users can create registrations"
ON registrations;

DROP POLICY IF EXISTS "Authenticated users can upload registration documents"
ON storage.objects;
